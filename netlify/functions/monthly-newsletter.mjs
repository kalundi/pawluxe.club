...
const ISSUE = {
  key: "2026-08",
  subject: "From Thandie: Move with purpose — August’s canine science letter",
...
async function sendEmail(apiKey, subscriber) {
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      from: { email: OWNER_EMAIL, name: "Thandie at Pawluxe Club" },
...
export default async () => {
  ...
  const subscribers = await databaseGet(supabaseSecret, "newsletter_subscribers?select=id,email,first_name&active=eq.true&newsletter=eq.true&order=created_at.asc");
  let sent = 0;
  for (const subscriber of subscribers) {
    const delivered = await databaseGet(supabaseSecret, `newsletter_deliveries?select=id&issue_key=eq.${ISSUE.key}&subscriber_id=eq.${subscriber.id}&limit=1`);
    if (delivered.length) continue;
    const providerStatus = await sendEmail(sendgridKey, subscriber);
    await logDelivery(supabaseSecret, { issue_key: ISSUE.key, subscriber_id: subscriber.id, email: subscriber.email, provider: "sendgrid", provider_status: providerStatus });
    sent += 1;
  }
  console.log(`Pawluxe ${ISSUE.key} newsletter complete: ${sent} sent, ${subscribers.length - sent} previously delivered.`);
};
